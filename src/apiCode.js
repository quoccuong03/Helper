...
export class apiCode {
   ...
   *    }
   */
  static getMessage(config = {}) {
    const {status_code = 500, fields, accepted_value, data, error_message, name, at, custom, format} = config
    let codeObj = utils._clone(this._code[status_code])
    ...
    if (!utils.isEmpty(custom)) codeObj = {...codeObj, ...custom}
