...
export class _date {
  ...
  static formatCodes = {
    ...
    D: 'this.Date.getShortDayName(m.getDay())', // get localized short day name
    j: 'm.getDate()',
    ...
    M: 'this.Date.getShortMonthName(m.getMonth())', // get localized short month name
    n: '(m.getMonth() + 1)',
   ...
   * @memberOf _date
   */
  static ceilDate(date, clone) {
    // handles invalid dates preventing the browser from crashing.
    if (isNaN(date.getTime())) return date

    if (clone) return this.ceilDate(this.clone(date))

    // get current hour before rounding
   ...
   * @memberOf _date
   */
  static clearTime(date, clone) {
    // handles invalid dates preventing the browser from crashing.
    if (isNaN(date.getTime())) return date

    if (clone) return this.clearTime(this.clone(date))
   ...
   * @memberOf _date
   */
  static add(date, interval, value) {
  ...
  static formatDate(date, separator = '') {
    const format = ['Y', 'm', 'd'].join(separator)
  ...
  static format(date, format) {
    ...
    const formatFn = this.createFormat(format)
    const scope = {
      String: _string,
      Date: _date
    }
  ...
  static createFormat(format) {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      if (!special && ch === '\\') special = true
        ...
        special = false
        code.push('\'' + _string.escape(ch) + '\'')
      } else {
        if (ch === '\n') code.push('\'\\n\'')
        else code.push(this.getFormatCode(ch))
      }
    }
    return utils.functionFactory('let m = arguments[0]; return ' + code.join('+'))
  }
   ...
   * @memberOf _date
   */
  static getFormatCode(character) {
    let f = this.formatCodes[character]
    if (f) {
    ...
    return f || ('\'' + _string.escape(character) + '\'')
  }
}